...
export default class CamController {
  ...
  constructor(cam: THREE.Camera, sock: Socket | null = null, direction: number = 1) {
    ...
    const self = this;
    const startMovement = function(code: number) {
    ...
    const endMovement = function(code: number) {
    ...
    const selectWeapon = function(keyCode: number) {
    ...
    const keyDownEvent = function(keyEvent: KeyboardEvent) {
      startMovement(keyEvent.keyCode);
    };

    const keyUpEvent = function(keyEvent: KeyboardEvent) {
