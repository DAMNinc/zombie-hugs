...
Game.prototype.joinPlayer = function (player) {
  ...
  if (Object.keys(this.players).length >= 2) {
    logger.info('%s cannot join game %s as player. Game is full', player.id, this.id);
  ...
  player.socket.on('fire', position => {
    logger.verbose('A zombie was added in game %s', this.id, position);

    const zombie = new Zombie(position, player.direction * -1, player.id);
  ...
  player.socket.on('pingpong', () => {
    logger.debug(`Ping from ${player.id}`);
    player.lastPing = Date.now();
  });
};

Game.prototype.exitPlayer = function (player) {
  logger.info(`Player ${player.id} exited`);
  ...
  for (let key of Object.keys(this.zombies)) {
    ...
    for (let otherKey in this.zombies) {
      ...
      if (zombie.isCollision(otherZombie)) {
        ...
        this.zombies[key].health--;
        if (this.zombies[key].health <= 0) {
          delete this.zombies[key];
        }

        this.zombies[otherKey].health--;
...
class Zombie {
  constructor(position, direction, firedByPlayer, health = 1) {
